...
public class MobileAction extends UIActions {
    ...
    public void Pinch(MobileElement elem) {
        ...
        // x and y is the middle location of the element on the screen
        ...
        finger2.press(new ElementOption().withElement(elem)
                        .withCoordinates(x, y + 100))
                .moveTo(new ElementOption().withElement(elem).withCoordinates(x, y + 10));
    ...
    @Step("Double Tap")
    public void duableTap(MobileElement elem)
    {
        wait.until(ExpectedConditions.visibilityOf(elem));
        TouchActions action = new TouchActions(mobileDriver);
        action.doubleTap(elem);
        action.perform();
